...
async def send_room_chat_enhanced(Msg, room_id, key, iv, region):
    ...
    return GeneRaTePk(packet, '1215', key, iv)
...
async def AuthClan(CLan_Uid, AuTh, K, V):
    ...
    return GeneRaTePk((await CrEaTe_ProTo(fields)).hex() , '1201' , K , V)
async def AutH_GlobAl(K, V):
    ...
    return GeneRaTePk((await CrEaTe_ProTo(fields)).hex() , '1215' , K , V)
...
async def GeT_Status(PLayer_Uid , K , V):
    ...
    if len(PLayer_Uid) == 10: Pk = f"080112090a05{PLayer_Uid}1005"
...
async def Msg_Sq(msg, owner, bot, K, V):
    ...
    return GeneRaTePk(proto_bytes.hex(), '1215', K, V)
...
def GeneRaTePk(Pk , N , K , V):
    ...
    header_length = DecodE_HeX(int(len(PkEnc) // 2))
    if len(header_length) == 2: HeadEr = N + "000000"
    elif len(header_length) == 3: HeadEr = N + "00000"
    elif len(header_length) == 4: HeadEr = N + "0000"
    elif len(header_length) == 5: HeadEr = N + "000"
    else: print('ErroR => GeneRatinG ThE PacKeT !! ')
    return bytes.fromhex(HeadEr + header_length + PkEnc)
...
def DecodeWhisperMessage(hex_packet):
    """Decode Whisper Message using hex packet"""
    try:
        ...
        print(f"Error in DecodeWhisperMessage: {e}")
        return None

async def RedZed_SendInv(bot_uid, target_uid, key, iv):
...
def encrypted_proto(encoded_hex):
    ...
    try:
        ...
        print(f"Error in encrypted_proto: {e}")
        return None
